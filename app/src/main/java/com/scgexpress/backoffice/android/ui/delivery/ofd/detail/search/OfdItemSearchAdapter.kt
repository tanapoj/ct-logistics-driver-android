...
class OfdItemSearchAdapter(val viewModel: OfdItemSearchViewModel) :
    RecyclerView.Adapter<OfdItemSearchAdapter.OfdItemViewHolder>() {
    companion object {
        const val VIEWTYPE_TRACKING = 0
    ...
    class OfdTrackingItemViewHolder(view: View, private val viewModel: OfdItemSearchViewModel) :
        OfdItemViewHolder(view) {
        ...
        fun bind(item: TrackingInfo, position: Int) = with(itemView) {
            ...
            tvRecipient.text = "ชื่อ-นามสกุล : " + item.recipientName

            val address = "ที่อยู่ : " + item.recipientAddress + ", " + item.recipientDistrict +
                    ", " + item.recipientProvince + " " + item.recipientZipcode
            tvAddress.text = address

            val tel = "โทร : " + item.recipientTel
            ...
            btnTel.setOnClickListener { viewModel.phoneCall(item.senderTel!!) }
            btnLocation.setOnClickListener {}
            btnCamera.setOnClickListener { viewModel.photo(item.trackingNumber!!) }
        }
    }

    class OfdBookingItemViewHolder(view: View, private val viewModel: OfdItemSearchViewModel) :
        OfdItemViewHolder(view) {
