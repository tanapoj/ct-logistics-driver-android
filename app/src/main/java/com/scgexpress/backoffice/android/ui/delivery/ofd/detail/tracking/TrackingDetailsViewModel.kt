...
class TrackingDetailsViewModel @Inject constructor(
    ...
    private var _data: MutableLiveData<TrackingInfo> = MutableLiveData()
    val data: LiveData<TrackingInfo>
        get() = _data
    ...
    private val mImagePlaceholder: Drawable? = ContextCompat.getDrawable(application, R.drawable.ic_placeholder)
    ...
    fun addPhoto(trackingNumber: String, filename: String) {
        addDisposable(repoNetwork.addPhoto(
            trackingNumber,
    ...
    fun checkCod() {
        if (_data.value!!.trackingNumber!!.startsWith("11"))
    ...
    private fun confirmOfdSent(codCollected: Double) {
        ...
        if (data.value == null) {
            ...
            trackingNo = data.value!!.trackingNumber!!
    ...
    private fun confirmOfdRetention(manifestID: String, otherReason: String) {
        ...
        deliveryParcel.add(
            DeliveryOfdParcel(
                trackingId = trackingNo, statusCode = retentionReason,
                datetimeInput = Utils.getServerDateTimeFormat(Utils.getCurrentTimestamp()), ofdNote = otherReason,
                latitude = latitude, longitude = longitude
            )
        )

        addDisposable(repoNetwork.scanOfd(manifestID, DeliveryOfdParcelList(deliveryParcel))
            .subscribeOn(Schedulers.io())
            .observeOn(AndroidSchedulers.mainThread())
            .subscribe({
    ...
    fun confirmOfdReStatus(rejectStatus: BookingRejectStatusModel, note: String) {
        ...
        addDisposable(repoNetwork.reStatus(manifestID, reStatus)
            .subscribeOn(Schedulers.io())
            .observeOn(AndroidSchedulers.mainThread())
            .subscribe({
    ...
    fun saveImage(myBitmap: Bitmap): String {
        ...
        return fileHelper.saveImageJpeg(context, Const.DIRECTORY_OFD_DETAIL_TRACKING, filename, myBitmap)?.absolutePath
            ?: ""
...
//        return ""
    ...
    fun upPhoto(transferUtility: TransferUtility, view: ImageView, filePath: String, trackingNo: String) {
    ...
    fun loadPhoto(imgView: ImageView, s3Client: AmazonS3Client, signatureImage: String, imageType: String) {
        try {
            var fileName = ""
            if (signatureImage != "") {
    ...
    private fun convertToBitmap(objectData: S3ObjectInputStream, imgView: ImageView, imageType: String) {
