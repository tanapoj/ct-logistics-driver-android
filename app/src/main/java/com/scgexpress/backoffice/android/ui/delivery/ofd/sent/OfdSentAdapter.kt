...
class OfdSentAdapter(private val viewModel: OfdSentViewModel) :
    RecyclerView.Adapter<OfdSentAdapter.ScanOfdViewHolder>() {
    ...
    class ScanOfdItemViewHolder(view: View) : ScanOfdViewHolder(view) {
        ...
        fun bind(item: Delivery, viewModel: OfdSentViewModel) = with(itemView) {
            tvCode.text = item.trackingNumber
            tvSender.text = item.senderCode + "\n" + item.senderName
            try {
                tvCodCollected.text = Utils.setCurrencyFormat(item.codCollected!!)
                tvCod.text = Utils.setCurrencyFormat(item.codAmount!!)
            } catch (ex: Exception) {
                tvCodCollected.text = "0"
                tvCod.text = "0"
            }
